...
export default function RewardsPage({ params }) {
  ...
  const [wallet, setWallet] = useState({ coins: 0, rupees: 0, pendingCoins: 0, pendingRupees: 0 });
  ...
  return (
    <div className={`min-h-screen bg-gray-900 text-white ${mooli.className}`}>
      ...
      <main className="p-6">
        {/* Wallet Section */}
        <div className="bg-gray-800 p-6 rounded-2xl mb-10 cursor-pointer hover:scale-[1.01] transition" onClick={() => setWalletOpen(!walletOpen)}>
          ...
          {walletOpen && (
            <div className="mt-4 border-t border-gray-700 pt-4 max-h-40 overflow-y-auto">
              {transactions.map(tx => (
                <div key={tx.id} className="flex justify-between bg-gray-700 p-2 rounded mb-2 text-sm">
                  <div>
              ...
              <button onClick={handleWithdraw} className="w-full py-2 mt-2 bg-yellow-500 text-gray-900 rounded-xl font-bold hover:bg-yellow-400">Withdraw</button>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {offers.map(offer => (
            <div key={offer.id} className="bg-gray-800 rounded-xl shadow overflow-hidden flex flex-col hover:ring-2 hover:ring-yellow-500/50 transition">
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <button
                  ...
                  className={`mt-auto px-4 py-2 rounded-full font-semibold transition ${
                    wallet.coins >= offer.cost ? "bg-yellow-500 text-gray-900 hover:bg-yellow-400" : "bg-gray-600 text-gray-400 cursor-not-allowed"
                  }`}
                >
                  {wallet.coins >= offer.cost ? (
                    <span className="flex items-center justify-center"><CheckCircle className="w-5 h-5 mr-1" /> Redeem</span>
                  ) : (
                    <span className="flex items-center justify-center"><XCircle className="w-5 h-5 mr-1" /> Need {offer.cost - wallet.coins} Coins</span>
                  )}
                </button>
              </div>
            </div>
